...
var Tsung = module.exports.Tsung = function(config) {
    ...
    /**
     * Adds a directory of which each file should be added.
     * We currently only support CSV files. Each `data.csv` file should have an accompanying `data.format` file.
     * This format file expresses how the CSV file should be parsed and what the column names are.
     * The first line is a comma seperated list that matches the CSV columns.
     * The second line defines how the file should be traversed (`iter` or `random`.)
     * The filename will be used as an internal identifier. Make sure this is unique accross all your data directories.
     * If you add a CSV file called `users.csv` which holds `username,password` combinations you can use the variable
     * `%%_users_username%%` in your requests.
     * 
     * @param {String} dir The path to the directory.
     */
    that.addDataDirectory = function(dir) {
        var files = fs.readdirSync(dir);
        for (var i = 0; i < files.length; i++) {
            var file = files[i];
            if (file.indexOf('.format') === file.length - 7) {
                var id = file.slice(0, file.length-7);
                var dataFile = dir + '/' + id + '.csv';
                if (fs.existsSync(dataFile)) {
                    // Read the format file so we can generate a list of dymnamic variables.
                    var format = fs.readFileSync(dir + '/' + file, 'utf-8');
                    var lines = format.split('\n');
                    var variables = lines[0].split(',');
                    var traversalOrder = lines[1];
                    dataFiles.push({'id': id, 'path': dataFile, 'variables': variables, 'traversalOrder': traversalOrder});
                }
            }
        }
    };
    ...
    that.to_xml = function() {
        ...
        for (var i = 0; i < dataFiles.length; i++) {
            ...
            xml += util.format('<option name="file_server" id="%s" value="%s" />', file.id, file.path);
        }
        xml += '</options>';
        ...
        for (var i = 0; i < sessions.length; i++) {
            xml += sessions[i].to_xml(dataFiles);
        }
        xml += '</sessions>';
...
var Session = function(name, probability) {
    ...
    that.currentTransaction = function(name) {
        if (actions.length > 0) {
    ...
    that.to_xml = function(dataFiles) {
        var xml = '';
        xml += util.format('<session name="%s" probability="%d" type="ts_http">', name, probability);

        // Dynamic variables (ex: files, random string generation)
        for (var i = 0; i < dataFiles.length; i++) {
            var file = dataFiles[i];
            xml += util.format('<setdynvars sourcetype="file" fileid="%s" delimiter="," order="%s">', file.id, file.traversalOrder);
            for (var v = 0; v < file.variables.length; v++) {
                xml += util.format('<var name="%s" />', file.id + '_' + file.variables[v]);
            }
            xml += '</setdynvars>';
        }
        // We'll also add a couple of variables that generate random data.
        xml += '<setdynvars sourcetype="random_string" length="13"><var name="random_string_short" /></setdynvars>';
        xml += '<setdynvars sourcetype="random_string" length="60"><var name="random_string_medium" /></setdynvars>';
        xml += '<setdynvars sourcetype="random_string" length="500"><var name="random_string_long" /></setdynvars>';
...
var Request = function(method, url, data) {
    ...
    that.to_xml = function() {
        var xml = util.format('<request subst="%s">', (url.indexOf('%') > 0 || variables.length > 0));
        ...
        if (method === 'POST') {
            ...
            contents = contents.replace(/%25/g, '%');
            xml += '<http url="' + url + '" method="' + method + '" version="1.1" contents="' + contents + '" />';
        } else {
            xml += '<http url="' + url + '" method="' + method + '" version="1.1" />';
        }
        xml += '</request>';
        return xml;
    };
